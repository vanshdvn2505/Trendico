...
function Login_Security() {
    ...
    const saveChanges = async () => {
        try {
            const response = await axios.post('http://localhost:7000/user/saveChanges', {username, email, phone, currPass, newPass, auth});
    ...
    const verifyCode = async () => {
        try {
            const response = await axios.post('http://localhost:7000/user/verifyCode', {auth, otp});
            console.log(response.data);
            setVerified(true);
        }
        catch(error){
    ...
    useEffect(() => {
        const sendEmail = async () => {
            try {
                const response = await axios.post('http://localhost:7000/user/sendEmail', {auth});
                console.log(response.data);
            }
            catch(error){
